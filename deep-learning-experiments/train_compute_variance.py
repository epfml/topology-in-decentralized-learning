...
config = {
    ...
    "weight_decay": 1e-4,
    "num_workers": 32,
    "variance_after_num_steps": 100,
    "variance repetitions": 100,
    "variance_computation_sample_frac": 0.01,
...
def main():
    ...
    variance_mask = (
        torch.rand_like(param_matrix[0]) < config["variance_computation_sample_frac"]
    )
    ...
    for lr_factor, fraction_of_training in config["step_decay"]:
        ...
        for local_batch_num in range(local_num_iterations):
            ...
            if should_eval(
                ...
                if config["inter_worker_distances"] != "none":
                    assert config["inter_worker_distances"] == "last iterate"
                    pyexr.write(
                        os.path.join(
                            output_dir, f"distance_matrix_{batch_num:07d}.exr"
                        ),
    ...
    # Compute variance
    param_checkpoint = param_matrix_ema
    momentum_checkpoint = optim_state.momentum

    gathered_models_for_cov = []

    for variance_rep in range(config["variance repetitions"]):
        print(f"Variance repetition {variance_rep}")

        param_matrix = param_matrix_ema = param_checkpoint.clone()
        optim_state = SGDState(momentum=momentum_checkpoint.clone())

        for step in range(config["variance_after_num_steps"]):
            with timer("data loading"):
                all_images, all_labels = next(train_iterator)
                all_images, all_labels = all_images.to(device), all_labels.to(device)
                xs = all_images.view(
            ...
            with timer("model update"):
                for worker, worker_stream, x, y, params, momentum in zip(
                    ...
                    with torch.cuda.stream(worker_stream):
                        ...
                        params.sub_(momentum, alpha=config["learning_rate"])

                for stream in worker_streams:
                    main_stream.wait_stream(stream)

            with timer("gossip averaging"):
                param_matrix = gossip_matrix(batch_num + 1 + step) @ param_matrix
        ...
        gathered_models_for_cov.append(param_matrix[:, variance_mask].cpu())

    torch.save(
        torch.stack(gathered_models_for_cov),
        os.path.join(output_dir, "variance_checkpoints.pickle"),
    )
